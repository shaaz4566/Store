...
function render(){
  ...
  $("#adminLogin").textContent="Sign out";
  ({dashboard,renderProducts,renderOrders,customers,settings}[tab]||dashboard)();
}
  ...
  try{
    ...
    if(e?.code === "auth/popup-blocked" || e?.code === "auth/popup-cancelled-by-user"){
      try{
        ...
        toast(redirectError?.message || "Google sign-in failed");
      }
    }else{
...
(async()=>{
  try{
    ...
    if(redirectResult?.user) await finishAdminUser(redirectResult.user);
  }catch(e){
    console.error("Google redirect sign-in:", e);
    toast(e?.message || "Google sign-in failed");
  }
})();
